...
- **정보 검색**: 키워드별 언론보도 효과성 및 검색 트렌드 분석
  - 네이버 뉴스 API를 활용한 언론보도 효과성 측정
  - 보도건수, 시계열 그래프, 보도내역 엑셀 다운로드
  - 네이버 검색 트렌드 시계열 그래프
  - 뉴스 리스트 정렬 기능 (제목, 언론사, 발행일 기준)
- **AI 분석 보고서 생성**: Perplexity AI를 활용한 종합 분석 보고서
  - 착안사항을 포함한 상세 분석
  - 마크다운 형식 보고서 생성
  - PDF 및 MD 파일 다운로드 지원
  - 참고문헌 링크 포함
- **분석 기간**: 최대 3개월 (자동 제한)
- **API 타임아웃 설정**: 관리자 페이지에서 설정 가능 (기본 5분)
...
#### 정보 검색 단계
1. 키워드 1개 입력 (최대 3개월 분석 기간)
2. 네이버 뉴스 수집 (최대 1000건, 950건 이상 시 경고 메시지)
3. 언론보도 효과성 측정
   - 보도건수 통계
   - 시계열 그래프 (분석 기간 전체 포함, 뉴스 없는 날 0건 표시)
   - 뉴스 리스트 (제목, 언론사, 발행일 정렬 가능)
   - 엑셀(CSV) 다운로드
4. 네이버 검색 트렌드 분석
   - 시계열 그래프
   - 검색량 통계
...
#### 화제성 분석 단계
1. 착안사항 입력 (선택사항)
2. Perplexity AI를 통한 종합 분석 보고서 생성
   - 마크다운 형식 보고서
   - 참고문헌 링크 포함
   - PDF 및 MD 파일 다운로드 지원
3. 보고서 미리보기 (HTML 렌더링)
4. PDF 다운로드 (한글 폰트 지원, 텍스트 복사 가능)
5. MD 다운로드 (원문 마크다운)
...
### 화제성 분석
- `POST /api/hot-topic-analysis/search-info` - 정보 검색 (언론보도 효과성, 검색 트렌드)
- `POST /api/hot-topic-analysis/generate-report` - AI 분석 보고서 생성
- `POST /api/hot-topic-analysis/convert-pdf` - PDF 변환
- `GET /api/hot-topic-analysis/download-pdf/:filename` - PDF 다운로드
- `GET/POST /api/perplexity-timeout` - Perplexity AI 타임아웃 설정
